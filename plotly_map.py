...
df = pd.read_csv(
    'jo.csv')
site_lat = tuple(df.lat.tolist())
site_lon = tuple(df.lon.tolist())
locations_name = tuple(df.text.tolist())
total = yourmomssklearn.get_percentage_increase_all_stations(2020, 2100)
print(type(total['waddington']))
print(type(site_lat), type(site_lon), type(locations_name))
...
total = yourmomssklearn.get_percentage_increase_all_stations(start, end)
...
for i in range(len(locations_name)):
    ...
    locations_name[i] = "{}: Projected to have {:.4f}% of increase from {} to {}".format(location.title(), total[location], start, end)
# print(locations_name)
# print(type(locations_name))
# print(total)
# print(type(total))
